...
def build_smpl_joint_components():
    ...
    try:
        _pkl_path = os.path.join(
            _ROOT, 'data/body_models/SMPL_python_v.1.1.0/smpl/models/SMPL_NEUTRAL.pkl'
        )
...
def compute_smpl_joints_from_verts(verts_m, tri_vids, baryc, J_reg):
    ...
    smpl_verts = (baryc[:, 0:1] * v0 + baryc[:, 1:2] * v1 + baryc[:, 2:3] * v2)  # [6890, 3]
...
SMPL_JOINT_NAMES = [
    'hip', 'L_hip', 'R_hip', 'spine1', 'L_knee', 'R_knee',
...
def main():
    ...
    ds = DatasetHMR(hparams.DATASET, dataset=args.dataset, is_train=(args.split == 'train'))
    ...
    for si, ds_idx in enumerate(indices):
        ...
        print(f'  Image: {img_w}x{img_h}, bbox_scale={bbox_scale:.3f}, center={bbox_center}')

        # ---- GT MHR vertices ----------------------------------------------
        gt_verts = to_numpy(item['vertices'])  # [V, 3] in metres
        print(f'\n--- GT MHR Vertices ---')
        print(f'  Shape: {gt_verts.shape}')
        print(f'  Range: x=[{gt_verts[:,0].min():.3f}, {gt_verts[:,0].max():.3f}], '
              f'y=[{gt_verts[:,1].min():.3f}, {gt_verts[:,1].max():.3f}], '
              f'z=[{gt_verts[:,2].min():.3f}, {gt_verts[:,2].max():.3f}]')
        print(f'  Body height (Y): {gt_verts[:,1].max() - gt_verts[:,1].min():.3f} m')

        # ---- GT SMPL joints (barycentric) ---------------------------------
        gt_smpl_joints = compute_smpl_joints_from_verts(gt_verts, tri_vids, baryc, J_reg)
        ...
        print(f'  Format: last_dim={skel_np.shape[-1]} (8=tz+quat, 16=col-major 4x4)')
        ...
        gt_smpl_from_fwd = compute_smpl_joints_from_verts(verts_m_out[0], tri_vids, baryc, J_reg)
        ...
        per_joint_diff = np.linalg.norm(gt_smpl_joints - gt_smpl_from_fwd, axis=1)
        ...
        if model_trainer is not None:
            ...
            total_mpjpe = 0
            for ji, name in enumerate(SMPL_JOINT_NAMES):
                print(f'    [{ji:2d}] {name:12s}: {per_joint_mpjpe[ji]:.1f} mm')
            ...
            if pred_verts.shape == gt_verts.shape:
                pve = np.linalg.norm(pred_verts - gt_verts, axis=1).mean() * 1000
                print(f'  Mean PVE: {pve:.1f} mm')
                print(f'  Max PVE:  {np.linalg.norm(pred_verts - gt_verts, axis=1).max()*1000:.1f} mm')
            else:
                print(f'  Shape mismatch: pred {pred_verts.shape} vs gt {gt_verts.shape}')
        ...
        if 'keypoints_orig' in item:
            ...
            for ji in [0, 1, 2, 4, 5, 10, 11, 19, 20]:
                ...
                print(f'    [{ji:2d}] proj=({jx:.0f},{jy:.0f}) kp=({kp_crop_x:.0f},{kp_crop_y:.0f}) '
