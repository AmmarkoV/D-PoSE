...
def parse_args():
    ...
    p.add_argument('--ckpt',
                   default=None,
                   help='Path to .ckpt checkpoint (omit for GT-only mode)')
    p.add_argument('--n',
                   type=int,
                   default=4,
    ...
    p.add_argument('--dataset',
                   default='agora-bfh',
                   help='Dataset name passed to DatasetHMR')
    ...
    p.add_argument(
        ...
        action='store_true',
        help=
        'Run visual unit tests (saved to <out_dir>/tests/) instead of sample loop'
    )
    return p.parse_args()
...
def render_mesh_stateless(
    ...
    rot_180x = trimesh.transformations.rotation_matrix(np.radians(180),
                                                       [1, 0, 0])
    ...
    light_pose = np.eye(4)
    light_pose[:3, 3] = [0, 1,
                         1]  # light position hint (direction light ignores t)
...
def run_visual_tests(args, hparams, ds, renderer, faces, smpl_faces,
    ...
    if gt_verts is None:
        print(
            '[SKIP] No vertices in dataset item. Run preconvert_mhr.sh first.')
        return
    ...
    center = item['center']
    bbox_center = center.numpy() if isinstance(
        center, torch.Tensor) else np.array(center)
    fl_val = (float(item['focal_length'][0]) if 'focal_length' in item else
              float((img_w_full**2 + img_h_full**2)**0.5))
    bbox_height = bbox_scale * 200.0
    fl_crop = fl_val * (224.0 / bbox_height)
    tz = 2.0 * fl_val / bbox_height
    half = bbox_scale * 100.0
    blank224 = lambda: (np.ones(
        (224, 224, 3), dtype=np.float32) * 0.8 * 255).astype(np.uint8)
    ...
    def test_01():
        ...
        ext = vmax - vmin
        y_range = ext[1]
        verdict = ('PASS — looks like metres' if 1.0 < y_range < 2.5 else
                   'FAIL — suspicious, check cm/m conversion')
        lines = [
    ...
    def test_02():
        ...
        axes[1].set_title(f'Centroid-centred\ncam_t=[0,0,{tz:.2f}]')
        axes[1].axis('off')

        ax3 = fig.add_subplot(1, 3, 3, projection='3d')
        step = max(1, len(v) // 600)
        sc = ax3.scatter(v[::step, 0],
                         v[::step, 2],
                         v[::step, 1],
                         s=1,
                         c=v[::step, 1],
        ...
        fig.suptitle(
            'TEST 02: Raw MHR verts centred at centroid\n'
            'Rendered mesh should show a human silhouette.',
            fontsize=10)
    ...
    def test_03():
        ...
        fig.suptitle(
            ...
            'Mesh should be centred; green circle marks its expected pixel.',
            fontsize=9)
    ...
    def test_04():
        ...
        axes[0].set_title('Input crop')
        axes[0].axis('off')
        axes[1].imshow(r_normal)
        axes[1].set_title('Normal (current code)')
        axes[1].axis('off')
        axes[2].imshow(r_yflip)
        axes[2].set_title('Y-flipped\n(v[:,1] *= -1)')
        axes[2].axis('off')

        fig.suptitle(
            'TEST 04: Y-flip hypothesis\n'
            'If mesh is upside-down, the Y-flipped panel should look correct.',
            fontsize=10)
    ...
    def test_05():
        ...
        for i, (lbl, vv) in enumerate(variants):
            ...
            axes[i].axis('off')
        fig.suptitle(
            'TEST 05: Axis-flip / swap variants\n'
            'Find the panel where the human stands upright.',
            fontsize=10)
    ...
    def test_06():
        ...
        axes[0].set_title(
            f'Crop render (224×224)\nfl_crop={fl_crop:.0f}  tz={tz:.3f}')
        axes[0].axis('off')
        axes[1].imshow(full_render)
        axes[1].set_title(
            f'Full-image render ({fw}×{fh})\nfl={fl_val:.0f}  cam_t={cam_t_full.round(3)}'
        )
        axes[1].axis('off')

        fig.suptitle(
            'TEST 06: Full-image render\n'
            'Mesh should appear at the person\'s location in the full image.',
            fontsize=10)
        path = _save(fig, 'test_06_full_image_render.png')
        print(
            f'[06] Full-image render  cam_t_full={cam_t_full.round(3)}  →  {path}'
        )
    ...
    def test_07():
        ...
        for ax, (lbl, vv) in zip(axes.flatten(), views):
            ...
            ax.axis('off')
        fig.suptitle(
            'TEST 07: Multi-view renders\n'
            'Front should show a standing/sitting person face-on.',
            fontsize=10)
    ...
    def test_08():
        ...
        smplx_model = smplx_lib.create(smplx_path,
                                       model_type='smplx',
                                       gender='neutral',
                                       use_pca=False,
                                       num_pca_comps=45,
                                       flat_hand_mean=True).cpu()
        with torch.no_grad():
            out = smplx_model(betas=torch.zeros(1, 10),
        ...
        def _rsx(vv):
            return render_mesh_stateless(vv,
                                         sx_faces,
                                         cam_t.copy(),
                                         blank224(), [fl_crop, fl_crop],
                                         mesh_color=(0.6, 0.8, 1.0))
        ...
        for ax, (lbl, vv) in zip(axes[1:], panels):
            ...
            ax.axis('off')
        fig.suptitle(
            f'TEST 08: SMPLX canonical mesh (ground-truth renderer orientation)\n'
            f'pelvis={sp.round(3)}  Y∈[{vmin[1]:.3f},{vmax[1]:.3f}]',
            fontsize=9)
    ...
    def test_09():
        ...
        joint_names = [
            ...
            'L_collar', 'R_collar', 'head', 'L_shldr', 'R_shldr', 'L_elbow',
        ...
        axes[0].set_title(
            'Mesh + SMPL joint regressor overlay\n(yellow = projected joints)')
        axes[0].axis('off')
        ...
        fig.suptitle(
            'TEST 09: SMPL joint regressor overlay\n'
            'Yellow dots should land on the correct anatomical locations.',
            fontsize=9)
    ...
    def test_10():
        ...
        cv2.drawMarker(r_centred, (112, 112), (0, 255, 0), cv2.MARKER_CROSS,
                       20, 2)
        cv2.drawMarker(r_aligned, (exp_px, exp_py), (0, 255, 0),
        ...
        axes[1].set_title(
            f'Pelvis at crop centre\n(tx=0, ty=0, tz={tz:.2f})\nGreen ✚ = expected pixel'
        )
        axes[1].axis('off')
        axes[2].imshow(r_aligned)
        axes[2].set_title(
            f'Pelvis at GT 2D pos\n(tx={tx:.3f}, ty={ty:.3f}, tz={tz:.2f})\nGreen ✚ = expected pixel'
        )
        axes[2].axis('off')
        fig.suptitle(
            'TEST 10: Camera placement\n'
            'Green cross = where the pelvis should project in crop pixels.',
            fontsize=10)
        path = _save(fig, 'test_10_camera_placement.png')
        print(
            f'[10] Camera placement  tx={tx:.3f} ty={ty:.3f} tz={tz:.3f}  →  {path}'
        )
    ...
    def test_11():
        ...
        mesh = trimesh.Trimesh(verts,
                               ...
                               process=False)
        rot180x = trimesh.transformations.rotation_matrix(
            np.radians(180), [1, 0, 0])
        mesh.apply_transform(rot180x)
        pymesh = _pyrender.Mesh.from_trimesh(mesh, smooth=False)

        cam_t = np.array([0., 0., tz], np.float32)
        cam_t_render = cam_t.copy()
        cam_t_render[0] *= -1

        scene = _pyrender.Scene(ambient_light=(0.8, 0.8, 0.8))
        scene.add(pymesh)
        cam_pose = np.eye(4)
        cam_pose[:3, 3] = cam_t_render
        scene.add(_pyrender.IntrinsicsCamera(fx=fl_crop,
                                             fy=fl_crop,
                                             cx=112,
                                             cy=112),
                  pose=cam_pose)
        ...
        axes[0].set_title(
            'Axis tetrahedron after 180°X rotation\n'
        ...
        axes[1].axis('off')
        fig.suptitle(
            'TEST 11: Renderer axis orientation\n'
            'If +Y does NOT point up, the mesh will appear upside-down.',
            fontsize=10)
    ...
    def test_12():
        ...
        r_mhr = render_mesh_stateless(v_mhr,
                                      faces,
                                      cam_t,
                                      black.copy(), [fl_crop, fl_crop],
                                      mesh_color=(0.85, 0.45, 0.45))
        r_smpl = render_mesh_stateless(v_smpl,
                                       smpl_faces,
                                       cam_t,
                                       black.copy(), [fl_crop, fl_crop],
                                       mesh_color=(0.45, 0.60, 0.90))
        ...
        overlay[mask_smpl,
                2] = 210  # blue channel → SMPL (and overlap → purple)

        diff = np.abs(r_mhr.astype(np.float32) -
                      r_smpl.astype(np.float32)).mean(2)
        ...
        axes[0].axis('off')
        axes[1].imshow(r_mhr)
        axes[1].set_title('MHR mesh (red)')
        axes[1].axis('off')
        axes[2].imshow(r_smpl)
        axes[2].set_title('SMPL mesh (blue)')
        axes[2].axis('off')
        ...
        axes[4].set_title(
            f'|MHR − SMPL| pixel diff\nmean(union)={mean_diff:.1f}')
        axes[4].axis('off')

        fig.suptitle(
            f'TEST 12: SMPL vs MHR rendering parity\n'
            f'IoU={iou:.4f}  union_px={union.sum()}  '
            f'MHR_px={mask_mhr.sum()}  SMPL_px={mask_smpl.sum()}',
            fontsize=10)
        path = _save(fig, 'test_12_smpl_mhr_parity.png')
        print(
            f'[12] SMPL vs MHR parity  IoU={iou:.4f}  mean_diff={mean_diff:.1f}  →  {path}'
        )
...
def dump_sample_diagnostics(
    ...
    focal_length_item = to_numpy(
        item['focal_length']) if 'focal_length' in item else None
    fl_x = float(
        focal_length_item[0]) if focal_length_item is not None else fl_val
    fl_y = float(
        focal_length_item[1]) if focal_length_item is not None else fl_val
    ...
    translation_np = to_numpy(
        item['translation']) if 'translation' in item else None
    ...
    tz_from_translation = float(
        translation_np[2]) if translation_np is not None else None
    ...
    # the "solved" depth.
    kp_orig_np = to_numpy(
        item['keypoints_orig']) if 'keypoints_orig' in item else None
    ...
    for ji, jname in enumerate(SMPL_JOINT_NAMES):
        ...
        conf = float(kp_orig_np[
            ji,
            2]) if kp_orig_np is not None and ji < kp_orig_np.shape[0] else 0.0
        u_ann = float(
            kp_orig_np[ji, 0]
        ) if kp_orig_np is not None and ji < kp_orig_np.shape[0] else None
        v_ann = float(
            kp_orig_np[ji, 1]
        ) if kp_orig_np is not None and ji < kp_orig_np.shape[0] else None
        ...
        if translation_np is not None:
            ...
            if u_ann is not None and abs(u_ann -
                                         cx_assumed) > 1.0 and conf > 0.1:
                denom_x = u_ann - cx_assumed
                tz_solved_x = fl_x * float(j3d_abs_i[0]) / denom_x if abs(
                    denom_x) > 1 else None
            if v_ann is not None and abs(v_ann -
                                         cy_assumed) > 1.0 and conf > 0.1:
                denom_y = v_ann - cy_assumed
                tz_solved_y = fl_y * float(j3d_abs_i[1]) / denom_y if abs(
                    denom_y) > 1 else None
        ...
        _tx_bbox = (float(pelvis_2d[0]) - cx_assumed
                    ) * tz_bbox / fl_x if pelvis_2d is not None else 0.0
        _ty_bbox = (float(pelvis_2d[1]) - cy_assumed
                    ) * tz_bbox / fl_y if pelvis_2d is not None else 0.0

        u_bbox, v_bbox = project_centred(j3d_cent, tz_bbox, _tx_bbox, _ty_bbox)

        err_bbox = None
        if u_ann is not None and u_bbox is not None and conf > 0.1:
            err_bbox = float(np.sqrt((u_bbox - u_ann)**2 +
                                     (v_bbox - v_ann)**2))
        ...
        u_transl, v_transl = project_centred(j3d_cent, float(cam_t_full[2]),
                                             float(cam_t_full[0]),
                                             float(cam_t_full[1]))
        if u_ann is not None and u_transl is not None and conf > 0.1:
            err_transl = float(
                np.sqrt((u_transl - u_ann)**2 + (v_transl - v_ann)**2))
        ...
        joint_report_rows.append({
            ...
            'u_ann':
            round(u_ann, 1) if u_ann is not None else None,
            'v_ann':
            round(v_ann, 1) if v_ann is not None else None,
            'conf':
            round(conf, 3),
            'u_bbox':
            round(u_bbox, 1) if u_bbox is not None else None,
            'v_bbox':
            round(v_bbox, 1) if v_bbox is not None else None,
            'err_bbox_px':
            round(err_bbox, 1) if err_bbox is not None else None,
            'u_transl':
            round(u_transl, 1) if u_transl is not None else None,
            'v_transl':
            round(v_transl, 1) if v_transl is not None else None,
            'err_transl_px':
            round(err_transl, 1) if err_transl is not None else None,
            'tz_solved_x':
            round(tz_solved_x, 3) if tz_solved_x is not None else None,
            'tz_solved_y':
            round(tz_solved_y, 3) if tz_solved_y is not None else None,
        })

    # Median solved tz across all joints (both x and y equations)
    all_tz_solved = tz_solved_per_joint_x + tz_solved_per_joint_y
    tz_solved_median = float(
        np.median(all_tz_solved)) if all_tz_solved else None
    ...
    if tz_solved_median is not None:
        for row in joint_report_rows:
            ...
            if u_ann is not None and conf > 0.1:
                ...
                if depth > 0:
                    u_s = cx_assumed + fl_x * (j_cent[0] +
                                               float(cam_t_full[0])) / depth
                    v_s = cy_assumed + fl_y * (j_cent[1] +
                                               float(cam_t_full[1])) / depth
    ...
    body_height_m = float(verts_max[1] -
                          verts_min[1])  # Y-axis extent (metres)
    ...
    try:
        ...
        full_img_diag = np.full((int(img_h_full), int(img_w_full), 3), 128,
                                np.uint8)

    def draw_joints_on_img(img,
                           joints_centred,
                           tz_cand,
                           tx_cand,
                           ty_cand,
        ...
        if label:
            _cv2.putText(out, label, (10, 30), _cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                         color, 2, _cv2.LINE_AA)
        return out
    ...
    fig_diag, axes_diag = plt.subplots(
        1,
        n_panels,
        figsize=(n_panels * int(img_w_full / 200), int(img_h_full / 200) + 1))
    ...
    _tx_b = (float(pelvis_2d[0]) -
             cx_assumed) * tz_bbox / fl_x if pelvis_2d is not None else 0.0
    _ty_b = (float(pelvis_2d[1]) -
             cy_assumed) * tz_bbox / fl_y if pelvis_2d is not None else 0.0
    img_tz_bbox = draw_joints_on_img(full_img_diag,
                                     smpl_joints_centred,
                                     tz_bbox,
                                     _tx_b,
                                     _ty_b,
                                     color=(0, 255, 0))
    mean_err_b = float(
        np.mean(reproj_errors_bbox)) if reproj_errors_bbox else float('nan')
    axes_diag[1].imshow(img_tz_bbox)
    axes_diag[1].set_title(
        f'tz_bbox={tz_bbox:.2f}m\n'
        f'2*fl/h  mean_err={mean_err_b:.0f}px',
        fontsize=8)
    ...
    img_tz_t = draw_joints_on_img(full_img_diag,
                                  smpl_joints_centred,
                                  float(cam_t_full[2]),
                                  float(cam_t_full[0]),
                                  float(cam_t_full[1]),
                                  color=(255, 128, 0))
    mean_err_t = float(np.mean(
        reproj_errors_transl)) if reproj_errors_transl else float('nan')
    axes_diag[2].imshow(img_tz_t)
    axes_diag[2].set_title(
        f'cam_t_full tz={cam_t_full[2]:.2f}m\n'
        f'pelvis3d+transl  err={mean_err_t:.0f}px',
        fontsize=8)
    ...
    if tz_solved_median is not None and tz_solved_median > 0:
        img_tz_s = draw_joints_on_img(
            ...
            color=(255, 0, 128))
        mean_err_s = float(np.mean(
            reproj_errors_solved)) if reproj_errors_solved else float('nan')
        axes_diag[3].imshow(img_tz_s)
        axes_diag[3].set_title(
            f'tz_solved={tz_solved_median:.2f}m\n'
            f'abs backproj ({len(all_tz_solved)}j)  err={mean_err_s:.0f}px',
            fontsize=8)
    else:
    ...
    summary_lines = [
        ...
        f'tz_transl    = {tz_from_translation:.3f} m'
        if tz_from_translation else 'tz_transl    = N/A',
        f'tz_solved    = {tz_solved_median:.3f} m   (median {len(all_tz_solved)}j)'
        if tz_solved_median else 'tz_solved    = N/A',
        f'tz_solved_σ  = {tz_solved_std:.3f}' if tz_solved_std else '',
        '',
        '── mean reproj err ──',
        f'tz_bbox      = {np.mean(reproj_errors_bbox):.1f} px'
        if reproj_errors_bbox else 'tz_bbox err  = N/A',
        f'tz_transl    = {np.mean(reproj_errors_transl):.1f} px'
        if reproj_errors_transl else 'tz_transl err= N/A',
        f'tz_solved    = {np.mean(reproj_errors_solved):.1f} px'
        if reproj_errors_solved else 'tz_solved err= N/A',
        '',
        '── vertex stats (orig) ──',
    ...
    diag_json = {
        ...
        'camera': {
            'fl_x': fl_x,
            'fl_y': fl_y,
            'cx_assumed': cx_assumed,
            'cy_assumed': cy_assumed,
        ...
        'bbox': {
            ...
            float(bbox_center[1]),
            'note':
            'scale is item[scale] which may include augmentation sc factor',
        },
        ...
        'reproj': {
            'tz_bbox_mean_px':
            float(np.mean(reproj_errors_bbox)) if reproj_errors_bbox else None,
            'tz_bbox_max_px':
            float(np.max(reproj_errors_bbox)) if reproj_errors_bbox else None,
            'tz_transl_mean_px':
            float(np.mean(reproj_errors_transl))
            if reproj_errors_transl else None,
            'tz_transl_max_px':
            float(np.max(reproj_errors_transl))
            if reproj_errors_transl else None,
            'tz_solved_mean_px':
            float(np.mean(reproj_errors_solved))
            if reproj_errors_solved else None,
            'tz_solved_max_px':
            float(np.max(reproj_errors_solved))
            if reproj_errors_solved else None,
        },
        ...
        'vertices': {
            'mhr_count':
            int(gt_verts_orig.shape[0]),
            'smpl_count':
            int(smpl_verts.shape[0]),
            'mhr_min':
            _j(verts_min),
            'mhr_max':
            _j(verts_max),
            ...
            _j(pelvis_3d),
            'note':
            ('pelvis_3d[2] near 0 → body-local verts (no abs. cam translation). '
        ...
        # -- item keys (shapes) --
        'item_keys': {
            k: (list(v.shape) if isinstance(v, (torch.Tensor,
                                                np.ndarray)) else str(type(v)))
    ...
    np.savez_compressed(
        ...
        kp_orig=kp_orig_np.astype(np.float32)
        if kp_orig_np is not None else np.zeros((24, 3), np.float32),
        # Camera
        focal_length_xy=np.array([fl_x, fl_y], np.float32),
        cam_ext=cam_ext_np.astype(np.float32)
        if cam_ext_np is not None else np.zeros((4, 4), np.float32),
        translation=translation_np.astype(np.float32)
        if translation_np is not None else np.zeros(3, np.float32),
        # Bbox
        bbox_params=np.array(
            [bbox_center[0], bbox_center[1], bbox_scale, bbox_height],
            np.float32),
        # tz candidates
    ...
    lines = [
        ...
        f'  tz_translation = {tz_from_translation:.4f} m   item[translation][2]'
        if tz_from_translation else '  tz_translation = N/A',
        f'  tz_solved_med  = {tz_solved_median:.4f} m   median reprojection ({len(all_tz_solved)} joint estimates)'
        if tz_solved_median else '  tz_solved_med  = N/A',
        f'  tz_solved_σ    = {tz_solved_std:.4f}' if tz_solved_std else '',
        f'',
        f'  VERDICT: ratio tz_solved/tz_bbox = {tz_solved_median/tz_bbox:.3f}'
        if tz_solved_median else
        '  VERDICT: cannot compute ratio (no tz_solved)',
        '─' * 80,
        'REPROJECTION ERRORS (mean over confident joints)',
        f'  tz_bbox        → {np.mean(reproj_errors_bbox):.1f} px'
        if reproj_errors_bbox else '  tz_bbox        → N/A',
        f'  tz_translation → {np.mean(reproj_errors_transl):.1f} px'
        if reproj_errors_transl else '  tz_translation → N/A',
        f'  tz_solved      → {np.mean(reproj_errors_solved):.1f} px'
        if reproj_errors_solved else '  tz_solved      → N/A',
        '─' * 80,
...
def main():
    ...
    _smpl_tri_vids = np.array(_faces)[_mapping['triangle_ids'].astype(
        np.int64)]  # [6890, 3]
    ...
    if args.ckpt:
        ...
        trainer_module = MHRTrainer.load_from_checkpoint(args.ckpt,
                                                         hparams=hparams)
    ...
    for sample_idx, ds_idx in enumerate(indices):
        ...
        for k in ('scale', 'center', 'orig_shape', 'focal_length'):
            if k in batch and isinstance(batch[k],
                                         torch.Tensor) and batch[k].dim() == 1:
        ...
        if gt_verts is not None:
            ...
            pelvis_3d = compute_smpl_pelvis(
                gt_verts_np)  # [3] metres, camera space
            gt_verts_np = gt_verts_np - pelvis_3d[
                np.newaxis]  # [V,3] pelvis-centred
            ...
            if 'translation' in item:
                _transl = to_numpy(
                    item['translation'])[:3]  # [tx, ty, tz] in metres
                # Absolute camera-space pelvis: body-root translation + local pelvis offset
                cam_t_full = (pelvis_3d + _transl).astype(np.float32)
                tx_full, ty_full, tz = float(cam_t_full[0]), float(
                    cam_t_full[1]), float(cam_t_full[2])
                print(
                    f'  translation={_transl.round(3)}  pelvis_local={pelvis_3d.round(3)}'
                )
            else:
            ...
            print(
                f'  cam_t_full={cam_t_full.round(3)}  fl={fl_val:.0f}  img={fw}x{fh}'
            )
            ...
            gt_render_full = render_mesh_stateless(
                ...
                camera_center=[fw // 2,
                               fh // 2],  # principal point at image centre
            )
            ...
            smpl_joints = compute_smpl_joints(
                gt_verts_np)  # [24,3] pelvis-centred
            for j3d in smpl_joints:
                depth = j3d[2] + tz  # ≈ tz for joints near z=0
                if depth <= 0:
                    continue
                jx_pix = int(
                    round(fw / 2 + fl_val * (j3d[0] + tx_full) / depth))
                jy_pix = int(
                    round(fh / 2 + fl_val * (j3d[1] + ty_full) / depth))
                if 0 <= jx_pix < fw and 0 <= jy_pix < fh:
                    _cv2.circle(gt_render_full, (jx_pix, jy_pix), 4,
                                (255, 255, 0), -1)
            ...
            _cv2.imwrite(_fi_path,
                         _cv2.cvtColor(gt_render_full, _cv2.COLOR_RGB2BGR))
        ...
        if gt_verts is not None:
            _smpl_j_orig = compute_smpl_joints(to_numpy(
                item['vertices']))  # BEFORE pelvis sub
            _smpl_j_centred = compute_smpl_joints(
                gt_verts_np)  # AFTER  pelvis sub
            _smpl_v_orig = compute_smpl_verts(to_numpy(item['vertices']))
            _pelvis_orig = compute_smpl_pelvis(to_numpy(
                item['vertices']))  # [3] pre-sub
            dump_sample_diagnostics(
                out_dir=args.out_dir,
                sample_idx=sample_idx,
                ds_idx=ds_idx,
                item=item,
                gt_verts_orig=to_numpy(
                    item['vertices']),  # [V,3] BEFORE pelvis sub
        ...
        if model is not None:
            ...
            if 'joints2d_smpl' in out:
                pred_kps2d_smpl = out['joints2d_smpl'][0].cpu().numpy(
                )  # [24, 2]

            try:
                pred_render = render_mesh_on_image(renderer, pred_verts_np,
                                                   pred_cam_t_np, img_crop,
        ...
        if pred_kps2d_smpl is not None:
            kp_img = draw_keypoints_on_image(img_crop,
                                             full_to_crop(pred_kps2d_smpl),
                                             (255, 80, 80),
                                             radius=4)
        ...
        gt_joints3d_np = to_numpy(
            gt_joints3d_item) if gt_joints3d_item is not None else None
        ...
        out_path = os.path.join(args.out_dir,
                                f'sample_{sample_idx:03d}_ds{ds_idx}.png')
        ...
        if gt_joints3d_np is not None:
            print(
                f'  GT joints3d range: {gt_joints3d_np.min():.3f} .. {gt_joints3d_np.max():.3f} m'
            )
        if pred_joints3d_np is not None:
            print(
                f'  Pred joints3d range: {pred_joints3d_np.min():.3f} .. {pred_joints3d_np.max():.3f} m'
            )
        if gt_verts is not None:
        ...
        if gt_kps_orig is not None:
            ...
            print(
                f'  keypoints_orig xy range: x=[{ko[:,0].min():.0f},{ko[:,0].max():.0f}]  '
                f'y=[{ko[:,1].min():.0f},{ko[:,1].max():.0f}]  '
                f'conf_min={ko[:,2].min():.2f}')
        if gt_kps_norm is not None:
            kn = to_numpy(gt_kps_norm)
            print(
                f'  keypoints (norm) xy range: [{kn[:,0].min():.2f},{kn[:,0].max():.2f}]'
            )
        if pred_kps2d_smpl is not None:
            print(
                f'  Pred kps2d_smpl range: x=[{pred_kps2d_smpl[:,0].min():.0f},{pred_kps2d_smpl[:,0].max():.0f}]  '
