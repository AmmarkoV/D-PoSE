        ...
        with tqdm(total=n, desc=dataset_name, unit="sample") as pbar:
            ...
            while i < n:
                ...
                for j in range(i, end):
                    ...
                    # Camera-space translation — matches the fix in
                    # dataset_wrapper._convert_smpl_to_mhr.  Without this the
                    # converter receives body-at-origin SMPL vertices and the
                    # fitted lbs_model_params[0:3] (MHR root translation)
                    # differs from the camera-space depth expected at inference,
                    # producing a systematic ~150px vertical joint offset.
                    if hasattr(ds, 'trans_cam'):
                        t = torch.from_numpy(ds.trans_cam[j].copy()).float()
                    else:
                        t = torch.zeros(3)
                    batch_transls.append(t)

                smpl_data = {
                    'pose':   torch.stack(batch_poses),   # [B, 165]
                    'betas':  torch.stack(batch_betas),   # [B, 10+]
                    'transl': torch.stack(batch_transls), # [B, 3]
                }
