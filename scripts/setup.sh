...
echo ""
echo -e "  3. ${YELLOW}Pre-convert datasets to MHR cache (one-time, run overnight):${NC}"
echo -e "     ${BLUE}./scripts/preconvert_mhr.sh --config MHRD-Pose/config_mhr.yaml${NC}"
echo -e "     (Use --resume to skip already-converted datasets if interrupted)"
echo ""
echo -e "  4. ${YELLOW}Start training:${NC}"
